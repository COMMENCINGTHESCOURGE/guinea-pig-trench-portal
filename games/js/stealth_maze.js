...
function initMap() {
    ...
    // ── Dim Mak Pressure Points — step on these to reconfigure the maze ──
    // Type 7 = pressure point. Stepping on it toggles nearby walls.
    // Same as acupuncture: one precise input, cascading system response.
    map[8][10] = 7;   // Cubicle pressure point — opens shortcut
    map[12][20] = 7;  // HR pressure point — creates new path
    map[10][33] = 7;  // Server room pressure point — reveals hidden corridor
}

// Pressure point wall toggles (which walls flip when a point is activated)
const PRESSURE_EFFECTS = {
    '8,10': [[6,7],[6,8],[6,9],[6,10],[6,11]], // Opens gap in cubicle wall
    '12,20': [[13,20],[14,20],[15,20],[16,20]], // Removes HR obstacle
    '10,33': [[11,31],[12,31],[13,31]],         // Opens server rack passage
};
const activatedPressurePoints = new Set();

function checkPressurePoint(pr, pc) {
    const key = `${pr},${pc}`;
    if (map[pr] && map[pr][pc] === 7 && !activatedPressurePoints.has(key)) {
        activatedPressurePoints.add(key);
        map[pr][pc] = 0; // Consume the pressure point

        // Toggle walls — dim mak: one touch, cascading effect
        const effects = PRESSURE_EFFECTS[key] || [];
        for (const [er, ec] of effects) {
            if (map[er] && map[er][ec] !== undefined) {
                map[er][ec] = map[er][ec] === 1 ? 0 : 1; // Toggle wall/floor
            }
        }

        // Visual feedback — pulse
        if (typeof ball !== 'undefined') ball.flash = 1.0;
        return true;
    }
    return false;
}
          ...
          for (let checkC = Math.floor(player.x - pickR); checkC <= Math.ceil(player.x + pickR); checkC++) {
            if (checkR >= 0 && checkR < ROWS && checkC >= 0 && checkC < COLS) {
              ...
              if (tdx * tdx + tdy * tdy < pickR * pickR) {
                ...
                if (map[checkR][checkC] === 7) { checkPressurePoint(checkR, checkC); }
              }
            }
          }
        }
...
function drawTile(c, r, color) {
    ...
    // ── Dim Mak Pressure Points — pulsing glow with meridian lines ──
    if (map[r] && map[r][c] === 7) {
        const pulse = 0.4 + 0.3 * Math.sin(gameTime * 0.08);
        ctx.fillStyle = `rgba(0,255,200,${pulse.toFixed(2)})`;
        ctx.beginPath();
        ctx.arc(c * TILE + TILE/2, r * TILE + TILE/2, TILE * 0.35, 0, Math.PI * 2);
        ctx.fill();

        // Meridian lines — faint connections to affected walls
        const key = `${r},${c}`;
        const effects = PRESSURE_EFFECTS[key] || [];
        ctx.strokeStyle = `rgba(0,255,200,${(pulse * 0.2).toFixed(2)})`;
        ctx.lineWidth = 0.5;
        for (const [er, ec] of effects) {
            ctx.beginPath();
            ctx.moveTo(c * TILE + TILE/2, r * TILE + TILE/2);
            ctx.lineTo(ec * TILE + TILE/2, er * TILE + TILE/2);
            ctx.stroke();
        }
    }
}
...
// ── Mycelium Fog — the explored area grows like fungi from the player's path ──
// Explored tiles persist (mycelium network). Currently visible tiles are bright.
// Recently explored tiles fade (the mycelium is alive but dimming).
// Unexplored tiles are dark. The crack pattern reveals the maze's own topology.
const exploredTiles = new Map(); // key: r*COLS+c, value: timestamp of first exploration
...
function drawFogOfWar(visible) {
    ...
    for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
            ...
            const wasExplored = exploredTiles.has(key);

            if (isVisible) {
                // Currently visible — no fog
                continue;
            } else if (wasExplored) {
                // Explored but not currently visible — mycelium memory
                // Fades over time but never fully disappears
                const timeSince = (now - exploredTiles.get(key)) * 0.001;
                const fadeAlpha = Math.min(0.65, 0.2 + timeSince * 0.03);

                // Melanin color shift — older exploration = darker (eumelanin absorption)
                const warmth = Math.max(0, 1 - timeSince * 0.05);
                const fogR = Math.round(4 + warmth * 8);
                const fogG = Math.round(4 + warmth * 4);
                const fogB = Math.round(8 + warmth * 2);
                ctx.fillStyle = `rgba(${fogR},${fogG},${fogB},${fadeAlpha.toFixed(2)})`;
                ctx.fillRect(c * TILE, r * TILE, TILE, TILE);

                // Mycelium tendrils — faint lines connecting explored tiles to their neighbors
                if (Math.random() < 0.02) {
                    const neighbors = [[r-1,c],[r+1,c],[r,c-1],[r,c+1]];
                    for (const [nr, nc] of neighbors) {
                        if (nr >= 0 && nr < ROWS && nc >= 0 && nc < COLS && exploredTiles.has(nr * COLS + nc)) {
                            ctx.strokeStyle = `rgba(0,180,160,${0.04 * warmth})`;
                            ctx.lineWidth = 0.5;
                            ctx.beginPath();
                            ctx.moveTo(c * TILE + TILE/2, r * TILE + TILE/2);
                            ctx.lineTo(nc * TILE + TILE/2, nr * TILE + TILE/2);
                            ctx.stroke();
                        }
                    }
                }
            } else {
                // Never explored — full dark
    ...
    // ── Fractal D readout — how much of the maze has been explored ──
    const exploredCount = exploredTiles.size;
    const totalFloor = ROWS * COLS - map.flat().filter(t => t === 1).length;
    const coverageD = exploredCount / Math.max(totalFloor, 1);

    // Display exploration dimension in HUD
    ctx.fillStyle = 'rgba(0,210,255,0.3)';
    ctx.font = '8px monospace';
    ctx.textAlign = 'left';
    ctx.fillText(`EXPLORED: ${(coverageD * 100).toFixed(0)}%  D≈${(1 + coverageD * 0.7).toFixed(2)}`, 10, canvas.height - 8);
}
