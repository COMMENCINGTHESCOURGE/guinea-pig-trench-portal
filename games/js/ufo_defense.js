...
let spritesAttempted = false;
const SPRITE_PATHS = {
    player_pink: '../assets/sprites/chain_refined/grief_warrior_sprite_sheet_256.png',
    player_green: '../assets/sprites/chain_refined/armored_defender_sprite_sheet_256.png',
    ufo: '../assets/sprites/chain_refined/void_runner_ship_256.png',
    boss: '../assets/sprites/chain_refined/kraken_game_render_256.png',
    boss_alt: '../assets/sprites/chain_refined/mecha_entity_alpha_v2_256.png',
    item: '../assets/sprites/chain_refined/geometric_core_geode_flux_256.png',
    alien: '../assets/sprites/chain_refined/aku_aku_mask_stylized_256.png'
};

// Sprite sheet frame regions (source rects within 256x256 sheets)
// Armored Defender: 3 labeled rows (IDLE/STAND, WALK, ATTACK), ~7 frames per row
// The labels take ~15% of the left side; frames occupy the rest
// Each frame is roughly 30x45px in the 256x256 sheet
const DEFENDER_FRAMES = {
    idle: [], walk: [], attack: []
};
(function() {
    // Row 0: IDLE/STAND — top ~33% of sheet, 7 frames across right portion
    // Row 1: WALK — middle ~33%, 7 frames
    // Row 2: ATTACK — bottom ~33%, 7 frames
    const cols = 7, labelW = 38, rowH = 75;
    const frameW = (256 - labelW) / cols;
    const rows = [{name:'idle', y:10}, {name:'walk', y:90}, {name:'attack', y:170}];
    for (const row of rows) {
        for (let c = 0; c < cols; c++) {
            DEFENDER_FRAMES[row.name].push({
                x: labelW + c * frameW, y: row.y, w: frameW, h: rowH
            });
        }
    }
})();

// Grief Warrior: 4 rows x 2 columns of poses
// Each cell is ~128x64 in the 256x256 sheet
const GRIEF_FRAMES = {
    idle: [], walk: [], attack: []
};
(function() {
    // Row 0: IDLE poses (2 variants) — top quarter
    // Row 1: DEFENDING/STAND (2 variants)
    // Row 2: KNEELING (2 variants)
    // Row 3: last row (2 variants)
    const cellW = 128, cellH = 64;
    // Use row 0 for idle, row 1 for walk, row 0+1 alternating for attack
    for (let c = 0; c < 2; c++) {
        GRIEF_FRAMES.idle.push({x: c * cellW, y: 0, w: cellW, h: cellH});
    }
    for (let c = 0; c < 2; c++) {
        GRIEF_FRAMES.walk.push({x: c * cellW, y: cellH, w: cellW, h: cellH});
    }
    for (let c = 0; c < 2; c++) {
        GRIEF_FRAMES.attack.push({x: c * cellW, y: cellH * 2, w: cellW, h: cellH});
    }
})();
...
(function loadSprites() {
    ...
    for (const key in SPRITE_PATHS) {
        ...
        img.onerror = function() {
            console.warn('Sprite failed to load:', key, SPRITE_PATHS[key]);
            loaded++;
            if (loaded >= total) spritesLoaded = true;
        };
...
// Helper: check if a sprite image is ready to draw
function spriteReady(key) {
    return SPRITES[key] && SPRITES[key].complete && SPRITES[key].naturalWidth > 0;
}

// --- Procedural Drawing Helpers ---
function drawPixelChar(x, y, w, h, color, facing, frame, isDodging) {
    const spriteKey = color === PINK_C ? 'player_pink' : 'player_green';
    if (spriteReady(spriteKey)) {
        const img = SPRITES[spriteKey];
        const isGreen = (spriteKey === 'player_green');
        const frames = isGreen ? DEFENDER_FRAMES : GRIEF_FRAMES;
        // Pick animation state
        let animState = 'idle';
        if (isDodging) animState = 'attack';
        else if (Math.abs(frame) > 0 && frame !== 0 && (typeof frame === 'number') && frame % 1 === 0 && frame !== 0) {
            // walkFrame > 0 means moving
            animState = frame > 0 ? 'walk' : 'idle';
        }
        const animFrames = frames[animState];
        const frameIdx = Math.floor(Math.abs(frame * 0.15)) % animFrames.length;
        const src = animFrames[frameIdx];
        X.save();
        X.translate(x, y);
        if (isDodging) { X.rotate(facing * (Math.abs(frame) % 12) * 0.04); X.globalAlpha = 0.6; }
        if (facing < 0) { X.scale(-1, 1); }
        // Draw sprite scaled to fit the character's game dimensions
        // Sprite is drawn with bottom-center at origin (x, y is feet position)
        const drawW = w * 1.8;  // slightly larger than hitbox for visual impact
        const drawH = h * 1.5;
        X.drawImage(img, src.x, src.y, src.w, src.h, -drawW/2, -drawH, drawW, drawH);
        X.restore();
        return;
    }
    // --- FALLBACK: procedural fillRect drawing ---
    X.save();
    X.translate(x, y);
...
function drawUFO(x, y, w, h, hpRatio, beamActive, frame) {
    ...
    if (spriteReady('ufo')) {
        ...
        // Draw the ship sprite scaled to UFO dimensions
        const drawW = w * 1.6;
        const drawH = h * 2.0;
        // Slight rotation wobble
        X.rotate(Math.sin(frame * 0.03) * 0.05);
        // Damage tint
        if (hpRatio < 0.5) {
            X.globalAlpha = 0.85;
            if (Math.random() < 0.3) spawnParticles(x + (Math.random()-0.5)*w, y, 1, '#ff6600', 1, 10);
        }
        X.drawImage(img, 0, 0, 256, 256, -drawW/2, -drawH/2, drawW, drawH);
        X.globalAlpha = 1;
        // Damage overlay
        if (hpRatio < 0.5) {
            X.fillStyle = RED + '33';
            X.beginPath();
            X.ellipse(0, 0, drawW*0.4, drawH*0.3, 0, 0, Math.PI * 2);
            X.fill();
        }
    } else {
    ...
    // Beam (always drawn same way — it's an effect, not a sprite)
    if (beamActive) {
...
function drawAlien(x, y, w, h, variant, frame, hpRatio) {
    X.save();
    X.translate(x, y);
    if (spriteReady('alien')) {
        const img = SPRITES['alien'];
        // Aku Aku mask as alien — single image, scaled to alien size
        const drawW = w * 1.6;
        const drawH = h * 1.4;
        // Bob slightly
        const bob = Math.sin(frame * 0.1) * 2;
        // Variant hue shift via filter (lightweight tinting)
        const hueShifts = [0, 90, 30, 210];
        const hue = hueShifts[variant % hueShifts.length];
        if (hue > 0) {
            X.filter = 'hue-rotate(' + hue + 'deg)';
        }
        X.drawImage(img, 0, 0, 256, 256, -drawW/2, -drawH + bob, drawW, drawH);
        X.filter = 'none';
    } else {
...
function drawMechBoss(x, y, w, h, frame, hpRatio) {
    ...
    if (spriteReady('boss')) {
        ...
        // Kraken boss — single image scaled to boss dimensions
        const drawW = w * 1.5;
        const drawH = h * 1.4;
        // Subtle sway
        X.rotate(Math.sin(frame * 0.03) * 0.03);
        // Damage flash
        if (hpRatio < 0.5) {
            if (Math.random() < 0.1) {
                spawnParticles(x + (Math.random()-0.5)*w, y - h*0.3, 2, '#ffaa00', 2, 15);
            }
            if (Math.random() < 0.15) X.globalAlpha = 0.7;
        }
        X.drawImage(img, 0, 0, 256, 256, -drawW/2, -drawH * 0.85, drawW, drawH);
        X.globalAlpha = 1;
        // Damage overlay
        if (hpRatio < 0.3) {
            X.fillStyle = RED + '22';
            X.fillRect(-drawW/2, -drawH * 0.85, drawW, drawH);
        }
    } else {
        // --- FALLBACK: procedural mech boss ---
        // Shadow
    ...
    // HP bar (always drawn, wide, at top)
    X.fillStyle = '#333';
...
function drawItem(x, y, type, frame) {
    ...
    if (spriteReady('item')) {
        const img = SPRITES['item'];
        // Geometric Core / Geode Flux — has multiple views in the sheet
        // Use different sub-regions for different item types
        // The sheet has 7 gems arranged: top row 3 (Geode), bottom row 4 (Flux)
        // Each gem is roughly 75x75 within the 256x256 sheet
        const itemRegions = {
            health: {x: 0, y: 10, w: 85, h: 90},     // Front geode (top-left)
            ammo:   {x: 85, y: 10, w: 85, h: 90},     // Top geode (top-center)
            shield: {x: 170, y: 10, w: 85, h: 90},    // Side geode (top-right)
            coin:   {x: 85, y: 120, w: 85, h: 90}     // Flux side (bottom-center)
        };
        const region = itemRegions[type] || itemRegions.coin;
        const drawSize = 24;
        // Glow effect behind sprite
        const glowColors = { health: '#ff333344', ammo: '#ffdd0044', shield: '#00d2ff44', coin: '#ffd70044' };
        const glow = X.createRadialGradient(0, 0, 2, 0, 0, 16);
        glow.addColorStop(0, glowColors[type] || '#ffffff44');
        glow.addColorStop(1, 'transparent');
        X.fillStyle = glow;
        X.fillRect(-16, -16, 32, 32);
        // Gentle rotation
        X.rotate(Math.sin(frame * 0.05) * 0.15);
        X.drawImage(img, region.x, region.y, region.w, region.h, -drawSize/2, -drawSize/2, drawSize, drawSize);
    } else {
