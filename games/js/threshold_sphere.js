const cv=document.getElementById('stc');
...
let flowTime=0,flowSpeed=0.025,bands=18,swing=30;

// LIVING THRESHOLD — breathes with 140 BPM ADSR
...
// THRESHOLD X — moves with the beat
function getThreshX(){
  const env=adsr();
  const swell=Math.sin(((performance.now()/1000*BPM/60/4)%1)*Math.PI);
  return CX+(env*swing*0.6)+(swell*swing*0.4);
}

// color functions — aware of threshold
function structuralPhase(sx,sy){
  const dr=Math.hypot(sx-CX,sy-CY)/R;
  const angle=Math.min(dr,1)*Math.PI;
  return((Math.cos(angle)*bands+flowTime)%1+1)%1;
}
function hue(h){
  h=((h%1)+1)%1;const x=1-Math.abs(h*6%2-1);
  if(h<1/6)return[1,x,0];if(h<2/6)return[x,1,0];if(h<3/6)return[0,1,x];
  if(h<4/6)return[0,x,1];if(h<5/6)return[x,0,1];return[1,0,x];
}
function getColor(sx,sy,melanin,alpha){
  const thX=getThreshX();
  const phase=structuralPhase(sx,sy);
  if(sx>thX){
    const[r,g,b]=hue(phase);
    const lum=(r+g+b)/3;
    const bright=melanin*(lum>0.4?1:0.22);
    return`rgba(0,${Math.round(bright*255)},${Math.round(bright*40)},${alpha})`;
  }
  const[r,g,b]=hue(phase);
  const bright=0.25+melanin*0.75;
  return`rgba(${Math.round(r*bright*255)},${Math.round(g*bright*255)},${Math.round(b*bright*255)},${alpha})`;
}
...
function updateFungi(){
  ...
  for(const tip of inwardTips){
    ...
    if(mag<0.45){const sc=7.0,b=0.19;const tx=Math.sin(tip.p[1]*sc)-b*(tip.p[0]*sc);const ty=Math.sin(tip.p[2]*sc)-b*(tip.p[1]*sc);const tz=Math.sin(tip.p[0]*sc)-b*(tip.p[2]*sc);const aD=vNrm([tx,ty,tz]);const sD=vNrm([-tip.p[0],-tip.p[1],-tip.p[2]]);const blend=Math.max(0,Math.min(1,(0.45-mag)/0.35));targetDir=vNrm(vLerp(sD,aD,blend));}
    else{targetDir=vNrm([-tip.p[0],-tip.p[1],-tip.p[2]]);}
  ...
  for(let i=0;i<inwardTips.length;i++){
    ...
    for(let j=0;j<outwardTips.length;j++){
      ...
      if(vMag(vSub(inwardTips[i].p,outwardTips[j].p))<0.05){
        ...
        contractCount++;document.getElementById('stcon').textContent=contractCount;break;
      }
    }
  }
...
function render(){
  const thX=getThreshX();
  const env=adsr();
  const transient=Math.exp(-(adsr())*4);
  G.fillStyle='#000308';G.beginPath();G.arc(CX,CY,R,0,Math.PI*2);G.fill();
  G.save();G.beginPath();G.arc(CX,CY,R,0,Math.PI*2);G.clip();

  // nebulae
  for(const n of NEBS){const[rx,ry,rz]=qRV([n.dx,n.dy,n.dz],Q);const[sx,sy]=prj(rx,ry,rz,ZM);if(!ic(sx,sy)||sx>thX)continue;const fd=Math.max(0,(rz+1)*0.5);const[nr,ng,nb]=n.col,sr=n.sz*(0.5+0.5*fd);const gg=G.createRadialGradient(sx,sy,0,sx,sy,sr);gg.addColorStop(0,`rgba(${nr},${ng},${nb},${n.al*fd*3})`);gg.addColorStop(0.55,`rgba(${nr},${ng},${nb},${n.al*fd})`);gg.addColorStop(1,'rgba(0,0,0,0)');G.fillStyle=gg;G.beginPath();G.arc(sx,sy,sr,0,Math.PI*2);G.fill();}
  for(const g of GALS){const[rx,ry,rz]=qRV([g.dx,g.dy,g.dz],Q);const[sx,sy]=prj(rx,ry,rz,ZM);if(!ic(sx,sy)||sx>thX)continue;G.save();G.translate(sx,sy);G.scale(1,0.36);G.fillStyle=`rgba(220,215,240,${g.al})`;G.beginPath();G.arc(0,0,g.sz,0,Math.PI*2);G.fill();G.restore();}
  // milky way
  for(let i=0;i<NMW;i++){const[rx,ry,rz]=qRV([MWDX[i],MWDY[i],MWDZ[i]],Q);const[sx,sy]=prj(rx,ry,rz,ZM);if(!ic(sx,sy)||sx>thX)continue;G.fillStyle=`rgba(215,205,188,${MWBR[i]*0.65})`;G.beginPath();G.arc(sx,sy,MWSZ[i],0,Math.PI*2);G.fill();}
  // stars
  for(let i=0;i<NS;i++){const[rx,ry,rz]=qRV([SDX[i],SDY[i],SDZ[i]],Q);const[sx,sy]=prj(rx,ry,rz,ZM);if(!ic(sx,sy))continue;const dr=Math.hypot(sx-CX,sy-CY)/R;const ef=dr>0.78?Math.pow(Math.max(0,1-(dr-0.78)/0.22),1.4):1;
    if(sx>thX){G.fillStyle=`rgba(0,${Math.round(SBR[i]*ef*255)},${Math.round(SBR[i]*ef*40)},${SBR[i]*ef})`;G.fillRect(sx,sy,SSZ[i]*1.8,SSZ[i]*1.8);}
    else{G.fillStyle=`rgba(${SCR[i]},${SCG[i]},${SCB[i]},${SBR[i]*ef})`;G.beginPath();G.arc(sx,sy,SSZ[i],0,Math.PI*2);G.fill();}}
  ...
  // density grid
  const cw=(R*2)/GDIM,ch2=(R*2)/GDIM,ox2=CX-R,oy2=CY-R;
  for(let gy=0;gy<GDIM;gy++)for(let gx=0;gx<GDIM;gx++){
    const i=gy*GDIM+gx,dI=densIn[i],dO=densOut[i],d=Math.max(dI,dO);
    if(d<0.02)continue;
    const sx=ox2+gx*cw+cw/2,sy=oy2+gy*ch2+ch2/2;
    if(!ic(sx,sy))continue;
    const col=getColor(sx,sy,dI>dO?0.3:0.7,Math.min(0.28,d*0.35));
    G.fillStyle=col;
    if(sx>thX)G.fillRect(sx-cw*0.7,sy-cw*0.7,cw*1.4,cw*1.4);
    else{G.beginPath();G.arc(sx,sy,cw*0.7,0,Math.PI*2);G.fill();}
  }

  // sea level
  const SEA_R=Math.sqrt(Math.max(0,F_R_OUTER*F_R_OUTER-SEA_Y*SEA_Y))*0.98;
  G.beginPath();let first=true;for(let a=0;a<=Math.PI*2+0.01;a+=0.1){const[rx,ry,rz]=qRV([Math.cos(a)*SEA_R,SEA_Y,Math.sin(a)*SEA_R],Q);const[sx,sy]=prj(rx,ry,rz,ZM);if(first){G.moveTo(sx,sy);first=false;}else G.lineTo(sx,sy);}
  G.closePath();G.fillStyle='rgba(15,40,70,0.12)';G.fill();G.strokeStyle='rgba(40,100,180,0.18)';G.lineWidth=0.7;G.stroke();

  // permanent splines
  G.lineCap='round';
  for(const s of permanentSplines){
    if(s.pts.length<2)continue;
    for(let i=1;i<s.pts.length;i++){
      const[r1x,r1y,r1z]=qRV(s.pts[i-1],Q),[r2x,r2y,r2z]=qRV(s.pts[i],Q);
      const[s1x,s1y]=prj(r1x,r1y,r1z,ZM),[s2x,s2y]=prj(r2x,r2y,r2z,ZM);
      if(!ic(s1x,s1y)&&!ic(s2x,s2y))continue;
      const prog=i/s.pts.length,mel=s.dir==='in'?prog:1-prog;
      const midx=(s1x+s2x)/2,midy=(s1y+s2y)/2;
      G.beginPath();G.moveTo(s1x,s1y);G.lineTo(s2x,s2y);
      G.strokeStyle=getColor(midx,midy,mel,0.12+prog*0.32);
      G.lineCap=midx>thX?'square':'round';
      G.lineWidth=(0.5+prog*0.45)*ZM;G.stroke();
      if(FR%6===0&&ic(midx,midy))stampDens(midx,midy,prog<0.5?'in':'out');
    }
  }
  // live tips
  for(const tip of [...inwardTips,...outwardTips]){
    ...
    for(let i=1;i<spl.length;i++){
      ...
      const[s1x,s1y]=prj(r1x,r1y,r1z,ZM),[s2x,s2y]=prj(r2x,r2y,r2z,ZM);
      if(!ic(s1x,s1y)&&!ic(s2x,s2y))continue;
      const prog=i/spl.length;const midx=(s1x+s2x)/2,midy=(s1y+s2y)/2;
      G.beginPath();G.moveTo(s1x,s1y);G.lineTo(s2x,s2y);
      G.strokeStyle=getColor(midx,midy,prog,0.08+prog*0.5);
      G.lineCap=midx>thX?'square':'round';G.lineWidth=1.1*ZM;G.stroke();
    }
    const[trx,try2,trz]=qRV(tip.p,Q);const[tsx,tsy]=prj(trx,try2,trz,ZM);
    if(ic(tsx,tsy)){
      const col=getColor(tsx,tsy,0.8,0.9);G.fillStyle=col;
      if(tsx>thX)G.fillRect(tsx-2*ZM,tsy-2*ZM,4*ZM,4*ZM);
      else{G.beginPath();G.arc(tsx,tsy,2*ZM,0,Math.PI*2);G.fill();}
    }
  }
  // contracts
  for(const c of contracts){
    ...
    const pulse=2.5+(env*1.8);const col=getColor(sx,sy,0.9,0.9*fade);G.fillStyle=col;
    if(sx>thX)G.fillRect(sx-pulse*ZM,sy-pulse*ZM,pulse*ZM*2,pulse*ZM*2);
    else{G.beginPath();G.arc(sx,sy,pulse*ZM,0,Math.PI*2);G.fill();}
  }

  // LIVING THRESHOLD LINE
  const glowW=2+env*4;
  G.beginPath();G.moveTo(thX,CY-R*1.1);G.lineTo(thX,CY+R*1.1);
  G.strokeStyle=`rgba(0,255,120,${0.5+env*0.4})`;G.lineWidth=glowW;
  G.setLineDash([6+env*4,4]);G.stroke();G.setLineDash([]);
  G.beginPath();G.moveTo(thX,CY-R*1.1);G.lineTo(thX,CY+R*1.1);
  G.strokeStyle=`rgba(0,255,80,${0.8+env*0.2})`;G.lineWidth=1;G.stroke();
  ...
  // iridescent shell rings
  for(let ring=0;ring<6;ring++){
    const shellR=R-ring*2.5,rOff=ring/6;
    const phase=structuralPhase(CX-shellR*0.7,CY);const[rc,gc,bc]=hue((phase+rOff)%1);
    G.beginPath();G.arc(CX,CY,shellR,0,Math.PI*2);
    G.strokeStyle=`rgba(${Math.round(rc*255)},${Math.round(gc*255)},${Math.round(bc*255)},${0.03+rOff*0.015})`;G.lineWidth=2.5;G.stroke();
  }
}

// controls
cv.addEventListener('mousedown',e=>{DRG=true;DB=e.button;LX=e.clientX;LY=e.clientY;AUTO=false;document.getElementById('stmode').textContent='\u25CB exploring';e.preventDefault();});
...
cv.addEventListener('wheel',e=>{ZM=Math.max(0.35,Math.min(5,ZM*(e.deltaY>0?0.92:1.087)));document.getElementById('stzm').textContent=ZM.toFixed(2);e.preventDefault();},{passive:false});
window.addEventListener('keydown',e=>{const sp=0.045;if(e.key==='ArrowLeft')Q=qNrm(qMul(qAx(0,1,0,sp),Q));if(e.key==='ArrowRight')Q=qNrm(qMul(qAx(0,1,0,-sp),Q));if(e.key==='ArrowUp')Q=qNrm(qMul(qAx(1,0,0,-sp),Q));if(e.key==='ArrowDown')Q=qNrm(qMul(qAx(1,0,0,sp),Q));if(e.key==='q'||e.key==='Q')Q=qNrm(qMul(qAx(0,0,1,-sp),Q));if(e.key==='e'||e.key==='E')Q=qNrm(qMul(qAx(0,0,1,sp),Q));if(e.key==='+'||e.key==='=')ZM=Math.min(5,ZM*1.1);if(e.key==='-')ZM=Math.max(0.35,ZM/1.1);AUTO=false;document.getElementById('stmode').textContent='\u25CB exploring';document.getElementById('stzm').textContent=ZM.toFixed(2);});
let TP={};
cv.addEventListener('touchstart',e=>{e.preventDefault();AUTO=false;document.getElementById('stmode').textContent='\u25CB exploring';for(let t of e.changedTouches)TP[t.identifier]={x:t.clientX,y:t.clientY};},{passive:false});
...
document.getElementById('stspd').addEventListener('input',e=>flowSpeed=parseFloat(e.target.value));
document.getElementById('stbnd').addEventListener('input',e=>bands=parseFloat(e.target.value));
document.getElementById('stswing').addEventListener('input',e=>swing=parseFloat(e.target.value));
