...
function startGame(){const dg=genDungeon(MW,MH);GRID=dg.grid;ROOMS=dg.rooms;mmCache=null;player=new Player(ROOMS[0][0],ROOMS[0][1]);bullets=[];particles=[];enemies=spawnE(ROOMS);cores=spawnCores(ROOMS);coreCount=0;gameTime=0;state='PLAYING';mdAccum=0;c.requestPointerLock()}
document.addEventListener('keydown',e=>{if(state==='MENU'){if(e.code==='Enter'||e.code==='Space')startGame()}if((e.code==='Escape'||e.code==='KeyP')&&state==='PLAYING'){state='PAUSED';document.exitPointerLock()}if((e.code==='Escape'||e.code==='KeyP')&&state==='PAUSED'){state='PLAYING';c.requestPointerLock()}if(state==='PLAYING'&&player){if(e.code==='KeyR'&&!player.reloading&&player.ammo<player.mammo){player.reloading=true;player.rt=1.5}if(e.code==='KeyF'&&player.bat>0)player.fl=!player.fl;if(e.code==='Space'&&player.ammo>0&&!player.reloading)fireQ=true}if((state==='DEAD'||state==='WIN')&&e.code==='Enter')state='MENU'});
let lastT=0;function loop(ts){const dt=Math.min(.05,(ts-lastT)/1000);lastT=ts;gameTime+=dt;
if(state==='MENU'){ctx.fillStyle='#020210';ctx.fillRect(0,0,W,H);ctx.fillStyle='#ffa';ctx.font='bold 60px Impact';ctx.textAlign='center';ctx.fillText('ELITE DUNGEON',W/2,H/2-40);ctx.fillStyle='#fc8';ctx.font='bold 36px Impact';ctx.fillText('SHOOTER',W/2,H/2+10);ctx.fillStyle='rgba(170,220,255,.7)';ctx.font='16px Arial';ctx.fillText('PRESS ENTER TO START',W/2,H/2+70);ctx.fillStyle='rgba(150,200,255,.4)';ctx.font='12px Arial';ctx.fillText('WASD Move | Mouse Aim | Click Shoot | R Reload | F Flashlight',W/2,H/2+100);ctx.textAlign='left'}
else if(state==='PLAYING'){const md=mdAccum;mdAccum=0;player.update(dt,keys,md);const shoot=fireQ;fireQ=false;if(shoot&&player.ammo>0&&!player.reloading){player.ammo--;player.recoil=36;player.mt=1;bullets.push(new Bullet(player.x,player.y,player.angle));for(let i=0;i<10;i++)particles.push(new Particle(player.x,player.y,(Math.random()-.5)*2.2,(Math.random()-.5)*2.2,[255,200+~~(Math.random()*55),100+~~(Math.random()*80)],.9))}bullets=bullets.filter(b=>{if(!b.update(dt))return false;for(const e of enemies){if(e.state==='DEAD')continue;if(Math.hypot(e.x-b.x,e.y-b.y)<getEnemyHitRadius(e)){e.hit(particles);if(e.state==='DEAD')player.kills++;return false}}return true});for(const e of enemies)e.update(dt,player,particles);enemies=enemies.filter(e=>e.state!=='DEAD'||Math.random()>.008);particles=particles.filter(p=>p.update(dt));
...
if(enemies.every(e=>e.state==='DEAD')&&enemies.length===0)state='WIN';if(player.hp<=0){player.hp=0;state='DEAD';document.exitPointerLock()}ctx.save();if(player.shakeT>0)ctx.translate(player.sx,player.sy);ctx.fillStyle='rgb(4,4,10)';ctx.fillRect(0,0,W,HH);ctx.fillStyle='rgb(16,16,22)';ctx.fillRect(0,HH,W,HH);castRays(player);renderCores(player);renderEnemies(player,enemies);renderGun(player);renderCH(player);renderHUD(player,enemies);renderMM(player,enemies);ctx.restore();if(player.hp<100){const da=Math.min(.55,(100-player.hp)/180),vg=ctx.createRadialGradient(W/2,H/2,H*.25,W/2,H/2,H*.75);vg.addColorStop(0,'rgba(180,0,0,0)');vg.addColorStop(1,`rgba(180,0,0,${da})`);ctx.fillStyle=vg;ctx.fillRect(0,0,W,H)}if(!pLocked){ctx.fillStyle='rgba(0,0,0,.6)';ctx.fillRect(0,0,W,H);ctx.fillStyle='rgba(255,255,255,.9)';ctx.font='bold 22px Impact';ctx.textAlign='center';ctx.fillText('CLICK TO CAPTURE MOUSE',W/2,H/2);ctx.textAlign='left'}}
