...
function checkAttack(attacker, defender) {
  ...
  if (boxOverlap(aBox, dBox)) {
    ...
    // Hit location determines effect multiplier.
    // Head hits (top 30%) = critical (pressure point). Body = normal. Legs = sweep.
    const hitY_rel = (aBox.y + aBox.h/2 - dBox.y) / dBox.h; // 0=top, 1=bottom
    let pressureMultiplier = 1.0;
    let pressureEffect = '';

    if (hitY_rel < 0.3) {
      // Head — dim mak critical point
      pressureMultiplier = 1.5;
      pressureEffect = 'PRESSURE POINT';
      // Freeze defender briefly (the pause mechanic)
      defender.stunTimer = (defender.stunTimer || 0) + 8;
    } else if (hitY_rel > 0.7) {
      // Legs — sweep, slower movement
      pressureMultiplier = 0.8;
      pressureEffect = 'SWEEP';
      defender.speedMult = Math.max(0.5, (defender.speedMult || 1.0) - 0.1);
      setTimeout(() => { defender.speedMult = 1.0; }, 1500);
    }

    const finalDmg = Math.round(dmg * pressureMultiplier);
    defender.takeDamage(finalDmg, attacker);
    attacker.hasHitThisAttack = true;
    const hitX = (aBox.x + aBox.w / 2 + dBox.x + dBox.w / 2) / 2;
    const hitY = (aBox.y + aBox.h / 2);

    // Meridian flash — show pressure point location
    const particleColor = pressureMultiplier > 1.0 ? '#00ffd2' : defender.blocking ? '#ffffff' : attacker.color;
    spawnHitParticles(hitX, hitY, particleColor, finalDmg > 10 ? 22 : 12);

    // Pressure point label
    if (pressureEffect) {
      spawnHitParticles(hitX, hitY - 20, '#00ffd2', 6);
    }
  }
}
