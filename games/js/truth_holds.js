...
// The 6 blob types from mantra_radio, now game creatures
const BLOB_TYPES = [
  { name:'CORE',     color:'#00e5cc', role:'neutral',  speed:0.8,  size:18, morphSpd:9,  hp:3, points:15,
    fill:'radial-gradient(ellipse at 40% 36%,#081f1e,#020c0b 80%)', border:'rgba(0,229,204,.6)' },
  { name:'SPECTRUM', color:'#b06cf7', role:'hostile',   speed:1.4,  size:16, morphSpd:7.5,hp:2, points:20,
    fill:'radial-gradient(ellipse at 55% 42%,#130d22,#070212 80%)', border:'rgba(176,108,247,.6)' },
  { name:'STATIONS', color:'#f0a500', role:'fleeing',   speed:1.8,  size:14, morphSpd:11, hp:1, points:30,
    fill:'radial-gradient(ellipse at 48% 55%,#1a1100,#0c0800 80%)', border:'rgba(240,165,0,.6)' },
  { name:'VOLUME',   color:'#f0365e', role:'hostile',   speed:1.2,  size:20, morphSpd:13, hp:4, points:25,
    fill:'radial-gradient(ellipse at 54% 36%,#1e0813,#0e040a 80%)', border:'rgba(240,54,94,.6)' },
  { name:'METER',    color:'#30e87a', role:'item',      speed:0.5,  size:12, morphSpd:8,  hp:1, points:10,
    fill:'radial-gradient(ellipse at 50% 58%,#051a0b,#020d05 80%)', border:'rgba(48,232,122,.6)' },
  { name:'LOGO',     color:'#dce8ff', role:'boss',      speed:0.6,  size:28, morphSpd:14, hp:8, points:50,
    fill:'radial-gradient(ellipse at 45% 38%,#111827,#040810 80%)', border:'rgba(220,232,255,.6)' },
];
...
function updateBlobs(dt) {
  ...
  for (const b of blobs) {
    ...
    switch (b.type.role) {
      case 'hostile':
        // Chase player if within 200 units
        if (dist < 200) {
...
// Draw blob as morphing shape on overlay canvas
function drawBlobSprite(b, sx, sy, size) {
  ...
  // HP pips for bosses/tough blobs
...
// Draw tendrils between nearby blobs
...
// Check projectile-blob collision
...
// ─── Five Worlds — procedural biome blending ───
const WORLDS=[
...
// ─── Grass map (procedural density per tile) ───
const grassMap = new Uint8Array(M * M);
// ─── Mushroom map (bioluminescent, world-zone dependent) ───
...
// ─── World generation ───
function generate(){
  // Procedural hash for deterministic randomness
  ...
  for(let i=0;i<M*M;i++){
    ...
    // Base terrain with world-zone variation
    ...
    // Procedural grass density (grows on flat green terrain)
    if (h >= 78 && h < 130) {
  ...
  // Spawn fauna — deer in Threshold zone, birds elsewhere
  ...
  for(let z=1;z<zMax;z+=zStep){
    ...
    for(let i=0;i<W;i++){
      ...
      if(hos<hiddenY[i]){
        ...
        // Procedural grass — add blade tips above terrain at close range
        if(z < 60 && grassMap[off] > 0) {
          const grassH = grassMap[off] / 255;
          const grassSway = Math.sin(plx*0.3 + wt*0.5) * grassH * 2;
          const bladeR = r * 0.6, bladeG = g * 1.3 + grassH * 30, bladeB = b * 0.5;
          // Blend grass into terrain color
        ...
        if(mushroomMap[off] > 0 && z < 80) {
          ...
          r += 0 * glowStr;
          g += 200 * glowStr;  // teal-green glow
          b += 255 * glowStr;  // cyan component
        }
...
function drawOverlay(){
  ...
  // Update blob count HUD
  const el = document.getElementById("blobCount");
  if(el) el.textContent = aliveCount;
...
function updateProjectiles(dt){
  ...
  for(let i=projectiles.length-1;i>=0;i--){
    ...
    if(isSGem||isDGem){
      state.gems+=10;playSnd("snd_mine");
...
// ─── HUD ───
function updateHUD(){
  document.getElementById("scoreVal").innerText=String(state.gems).padStart(3,"0");
  const pct=Math.min(100,(state.gems/300)*100);
  ...
  mf.style.background=state.muzzleFlash>0.02?`rgba(0,255,157,${state.muzzleFlash*0.12})`:"rgba(0,255,157,0)";
}
...
function triggerAscension(){
  ...
  document.getElementById("overlay-score").innerText=`HARVESTED — ${state.gems} UNITS`;
...
function triggerEnd(){
  ...
  document.getElementById("overlay-score").innerText=`HARVESTED — ${state.gems} / 300 UNITS`;
}
...
function update(dt){
  ...
  if(state.layer==="SURFACE"&&state.gems>=300){
...
// ─── Fauna rendering (on overlay canvas) ───
function drawFauna() {
  for (const f of fauna) {
    if (!f.alive) continue;
    // Project to screen
    ...
    oCtx.save();
    if (f.type === 'deer') {
      // Procedural deer — body + legs + head
