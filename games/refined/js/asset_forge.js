...
function drawPlayPhase() {
  ...
  // Stars
  for (let i = 0; i < 80; i++) {
    const sx = ((SEEDS[i%SEEDS.length]*3+i*47-cam*0.1)%W+W)%W;
    const sy = (SEEDS[i%SEEDS.length]*7+i*31)%(H*0.6);
    ctx.fillStyle = `rgba(200,220,255,${0.2+Math.sin(g.time+i)*0.15})`;
    ctx.fillRect(sx, sy, 1 + i%2, 1 + i%2);
  }

  // Platforms
  for (const plat of g.platforms) {
    const px = plat.x - cam, py = plat.y;
    if (px + plat.w < 0 || px > W) continue;
    ctx.fillStyle = plat.color;
    ctx.fillRect(px, py, plat.w, plat.h);
    ctx.strokeStyle = 'rgba(0,255,210,0.1)';
  ...
  for (const e of g.entities) {
    if (e.type === 'prop') {
      const ex = e.x - cam;
      if (ex + e.w < -50 || ex > W + 50) continue;
      drawAsset(e.asset, ex, e.y - e.h, e.w, e.h, 0);
    }
  }
  ...
  for (const e of g.entities) {
    if (e.type === 'item' && !e.collected) {
      ...
      ctx.save();
      ctx.globalAlpha = 0.3;
      ctx.shadowColor = `rgb(${e.asset.sprite.palette[0].join(',')})`;
      ctx.shadowBlur = 15;
      ctx.fillStyle = `rgb(${e.asset.sprite.palette[0].join(',')})`;
      ctx.beginPath(); ctx.arc(ex + e.w/2, e.y - e.h/2 + bob, e.w/2 + 4, 0, Math.PI*2); ctx.fill();
      ctx.restore();

      drawAsset(e.asset, ex, e.y - e.h + bob, e.w, e.h,
        Math.floor(g.time * 2) % FORGE_ANGLES);
    }
  }
  ...
  for (const e of g.entities) {
    if (e.type === 'enemy' && e.alive) {
      const ex = e.x - cam;
      if (ex < -50 || ex > W + 50) continue;
      drawAsset(e.asset, ex, e.y - e.h, e.w, e.h, e.facing);

      // HP bar
      ctx.fillStyle = 'rgba(255,0,0,0.4)';
      ctx.fillRect(ex, e.y - e.h - 6, e.w, 3);
      ctx.fillStyle = '#ff2244';
      ctx.fillRect(ex, e.y - e.h - 6, e.w * (e.hp / (Math.round(e.asset.sprite.complexity*3)+1)), 3);
    }
  }
  ...
  // Attack visual
  if (p.attackTimer > 0.15) {
    const atkX = px + (p.facing === 2 ? p.w : -20);
    ctx.save();
    ctx.globalAlpha = 0.5;
    ctx.strokeStyle = '#00ff9d';
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.arc(atkX + 10, p.y + p.h/2, 18, 0, Math.PI*2); ctx.stroke();
    ctx.restore();
  }

  drawAsset(p.asset, px, p.y, p.w, p.h, p.facing);
